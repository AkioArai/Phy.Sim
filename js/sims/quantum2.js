...
sterngerlach:{
  ...
  step(s,dt,p){
    ...
    const скорость=Math.max(0.8,p.rate/60);              // при частом счёте летят быстрее
    for(const a of s.летят) a.t+=dt*скорость;
    while(s.летят.length && s.летят[0].t>=1){ const a=s.летят.shift(); this.засчитать(s,a); }
    // лишние в полёте не теряем, а сразу засчитываем: статистика не должна зависеть от анимации
    while(s.летят.length>200) this.засчитать(s,s.летят.shift());
  },
