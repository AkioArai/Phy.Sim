/* Проверка учебной структуры пособия.
   ...
   4. Имена задач уникальны и не меняются без ведома.

   5. Приёмы выводов. У каждого шага названо, какой математикой он сделан,
      и каждое такое имя есть в справочнике js/ops.js. Статья, которой не
      пользуется ни один шаг, — тоже ошибка: справочник не должен
      разрастаться в отдельный курс математики, от которого мы отказались.
...
const { ПРИЁМЫ, ГРУППЫ_ПРИЁМОВ } = new Function(читать('js/ops.js') + '; return {ПРИЁМЫ,ГРУППЫ_ПРИЁМОВ};')();
...
/* ---------------- 5. Приёмы выводов ---------------- */
{
  ...
  let шагов = 0;
  for (const t of ALL)
    (t.derivations || []).forEach((d, i) => d.steps.forEach((s, k) => {
      шагов++;
      const где = `${t.id}, вывод ${i + 1}, шаг ${k + 1}`;
      if (!Array.isArray(s.op) || !s.op.length) { беды.push(`${где}: не назван приём (поле op)`); return; }
      if (new Set(s.op).size !== s.op.length) беды.push(`${где}: приём назван дважды`);
      for (const o of s.op) {
        if (!ПРИЁМЫ[o]) беды.push(`${где}: приёма «${o}» нет в js/ops.js`);
        else счёт[o] = (счёт[o] || 0) + 1;
      }
    }));
  const группы = new Set(ГРУППЫ_ПРИЁМОВ.map(g => g.id));
  for (const [id, п] of Object.entries(ПРИЁМЫ)) {
    if (!счёт[id]) беды.push(`«${id}»: статья есть, а ни один шаг этим приёмом не сделан`);
    if (!группы.has(п.группа)) беды.push(`«${id}»: неизвестная группа «${п.группа}»`);
    for (const поле of ['кратко', 'имя', 'что', 'законно', 'ломается'])
      if (!п[поле] || !String(п[поле]).trim()) беды.push(`«${id}»: пустое поле ${поле}`);
    // метка стоит под шагом в узкой колонке телефона
    if (п.кратко && п.кратко.length > 22) беды.push(`«${id}»: метка «${п.кратко}» длиннее 22 знаков`);
  }
  итог(`у всех ${шагов} шагов выводов назван приём, статьи — ${Object.keys(ПРИЁМЫ).length}, лишних нет`, беды);
}
