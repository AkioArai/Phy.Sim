// Сводная проверка перед выпуском: загрузка без ошибок, все 78 симуляций
...
(async () => {
  ...
  for (const [label, url] of [['исходники', 'http://localhost:8971/'],
    ...
    ok('78 симуляций', counts.sims === 78, counts);
    ok('темы и задачи на месте', counts.topics >= 36 && counts.problems >= 394, counts);
    ...
    ok('схемы размечены', оси.схем === 44, оси.схем);
    ...
    ok('блоки пособия на экране',
       урок.зачем === 1 && урок.предпосылки === 3 && урок.выводов === 3 &&
